...
void LegacyRepo::store_order(Order& order)  throw (InvalidFileNotOpenException){
    ofstream fout;
    fout.open("OrderLegacy.txt", ios::app);
    if (fout.is_open()) {
        fout << order;
        fout.close();
    }else{throw InvalidFileNotOpenException();}
}
vector <Order> LegacyRepo::get_order()  throw (InvalidFileNotOpenException){
    vector <Order> orders;
    ifstream fin;
